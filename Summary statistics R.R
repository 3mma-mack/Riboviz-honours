...
#woops i got a thing wrong and have to do it manually  
gene_reads_5UTR_CDS.wt.AT.CHX.ribo.11_s.h5 <- gene_reads_5UTR_CDS.wt.AT.CHX.ribo.11_s.h5%>% mutate(ratio = fiveUTR_reads_per_base/CDS_reads_per_base,
                                                                   Treatment = 'AT.CHX', 
                                                                   Sample = 'wt.AT.CHX.ribo.11_s',
                                                                   .keep = 'all')
gene_reads_5UTR_CDS.wt.AT.CHX.ribo.13_s.h5 <- gene_reads_5UTR_CDS.wt.AT.CHX.ribo.13_s.h5%>% mutate(ratio = fiveUTR_reads_per_base/CDS_reads_per_base,
                                                                                               Treatment = 'AT.CHX', 
                                                                                               Sample = 'wt.AT.CHX.ribo.13_s',
                                                                                               .keep = 'all')
gene_reads_5UTR_CDS.wt.AT.noCHX.ribo.11_s.h5 <- gene_reads_5UTR_CDS.wt.AT.noCHX.ribo.11_s.h5%>% mutate(ratio = fiveUTR_reads_per_base/CDS_reads_per_base,
                                                                                                     Treatment = 'AT.noCHX',
                                                                                                     Sample = 'wt.AT.noCHX.ribo.11_s',
                                                                                                     .keep = 'all')
gene_reads_5UTR_CDS.wt.AT.noCHX.ribo.13_s.h5 <- gene_reads_5UTR_CDS.wt.AT.noCHX.ribo.13_s.h5%>% mutate(ratio = fiveUTR_reads_per_base/CDS_reads_per_base,
                                                                                                  Treatment = 'AT.noCHX',
                                                                                                  Sample = 'wt.AT.noCHX.ribo.13_s',
                                                                                                  .keep = 'all')
gene_reads_5UTR_CDS.wt.AT.ribo.3_s.h5 <- gene_reads_5UTR_CDS.wt.AT.ribo.3_s.h5%>% mutate(ratio = fiveUTR_reads_per_base/CDS_reads_per_base,
                                                                                                Treatment = 'AT', 
                                                                                         Sample = 'wt.AT.ribo.3_s',
                                                                                                .keep = 'all')
gene_reads_5UTR_CDS.wt.AT.ribo.4_s.h5 <- gene_reads_5UTR_CDS.wt.AT.ribo.4_s.h5%>% mutate(ratio = fiveUTR_reads_per_base/CDS_reads_per_base,
                                                                                                Treatment = 'AT', 
                                                                                         Sample = 'wt.AT.ribo.4_s',
                                                                                                .keep = 'all')
gene_reads_5UTR_CDS.wt.noAT.CHX.ribo.11_s.h5 <- gene_reads_5UTR_CDS.wt.noAT.CHX.ribo.11_s.h5%>% mutate(ratio = fiveUTR_reads_per_base/CDS_reads_per_base,
                                                                                                       Treatment = 'noAT.CHX', 
                                                                                                       Sample = 'wt.noAT.CHX.ribo.11_s',
                                                                                                       .keep = 'all')
gene_reads_5UTR_CDS.wt.noAT.CHX.ribo.13_s.h5 <- gene_reads_5UTR_CDS.wt.noAT.CHX.ribo.13_s.h5%>% mutate(ratio = fiveUTR_reads_per_base/CDS_reads_per_base,
                                                                                                       Treatment = 'noAT.CHX',
                                                                                                       Sample = 'wt.noAT.CHX.ribo.13_s',
                                                                                                       .keep = 'all')
gene_reads_5UTR_CDS.wt.noAT.noCHX.ribo.11_s.h5 <- gene_reads_5UTR_CDS.wt.noAT.noCHX.ribo.11_s.h5%>% mutate(ratio = fiveUTR_reads_per_base/CDS_reads_per_base,
                                                                                                         Treatment = 'noAT.noCHX', 
                                                                                                         Sample = 'wt.noAT.noCHX.ribo.11_s',
                                                                                                         .keep = 'all')
gene_reads_5UTR_CDS.wt.noAT.noCHX.ribo.13_s.h5 <- gene_reads_5UTR_CDS.wt.noAT.noCHX.ribo.13_s.h5%>% mutate(ratio = fiveUTR_reads_per_base/CDS_reads_per_base,
                                                                                                      Treatment = 'noAT.noCHX',
                                                                                                      Sample = 'wt.noAT.noCHX.ribo.13_s',
                                                                                                      .keep = 'all')
gene_reads_5UTR_CDS.wt.noAT.ribo.3_s.h5 <- gene_reads_5UTR_CDS.wt.noAT.ribo.3_s.h5%>% mutate(ratio = fiveUTR_reads_per_base/CDS_reads_per_base,
                                                                                                  Treatment = 'noAT', 
                                                                                             Sample = 'wt.noAT.ribo.3_s',
                                                                                                  .keep = 'all')
gene_reads_5UTR_CDS.wt.noAT.ribo.4_s.h5 <- gene_reads_5UTR_CDS.wt.noAT.ribo.4_s.h5%>% mutate(ratio = fiveUTR_reads_per_base/CDS_reads_per_base,
                                                                                                  Treatment = 'noAT',
                                                                                             Sample = 'wt.noAT.ribo.4_s', 
                                                                                                  .keep = 'all')
...
genes_of_interest <- All_samples_one_df %>% filter(All_samples_one_df$Gene %in% c('SPCC1393.08.1', 'SPCC188.09c.1', 'SPAC2E12.05.1', 'SPCC777.03c.1'))
...
ggplot(condition_change,aes(x = no_AT_Ratio, y = with_AT_Ratio))+
  geom_jitter(color = 'red')+
  geom_point(data=genes_of_interest, colour="green") +
  geom_text(data=genes_of_interest, label= genes_of_interest$Gene, vjust=1.5, size = 2.5)+
  geom_text(data=condition_change, label = condition_change$Gene, vjust=2, hjust=1.5, size = 2)+
